defmodule SnippetSaverWeb.SettingLive.ColourTable do
  @moduledoc false
  ...
  def fields do
    [
      ...
      actions: %{
        ...
        renderer: fn c ->
          id = if is_struct(c), do: c.id, else: c[:id] || get_in(c, [:actions, :id])
          archived? = if is_struct(c), do: c.archived, else: Map.get(c, :archived, false)
          ...
          >
            ...
            <.button
              ...
              phx-click="archive"
              phx-value-id={@id}
              data-confirm="Archive this colour? It will be hidden from patient pickers."
            >
