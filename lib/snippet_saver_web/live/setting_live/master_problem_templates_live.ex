defmodule SnippetSaverWeb.SettingLive.MasterProblemTemplatesLive do
  use SnippetSaverWeb, :live_view

  use LiveTable.ExportHelpers, schema: SnippetSaver.Settings.MasterProblemTemplate
  ...
  use LiveTable.TableComponent,
    table_options:
      LiveTable.TableConfig.get_table_options(
        SnippetSaverWeb.SettingLive.MasterProblemTemplateTable.table_options()
      )
  ...
  alias SnippetSaverWeb.SettingLive.{CatalogTableSupport, MasterProblemTemplateTable}

  def fields, do: MasterProblemTemplateTable.fields()
  def filters, do: MasterProblemTemplateTable.filters()

  def list_resources(fields, options, schema \\ MasterProblemTemplate) do
    CatalogTableSupport.list_resources(
      fields,
      options,
      schema,
      MasterProblemTemplateTable.table_options()
    )
  end

  def stream_resources(fields, options, schema \\ MasterProblemTemplate) do
    CatalogTableSupport.stream_resources(
      fields,
      options,
      schema,
      MasterProblemTemplateTable.table_options()
  ...
  def get_merged_table_options do
    CatalogTableSupport.get_merged_table_options(MasterProblemTemplateTable.table_options())
  end

  @new_drawer_id "new-master-problem-template-drawer"
  @view_drawer_id "view-master-problem-template-drawer"

  @impl true
  def mount(_params, _session, socket) do
    {:ok,
     socket
     |> assign(:page_title, "Master problem templates")
     |> assign(:active_page, "settings_patient")
     |> assign(:detail_form, nil)
     |> assign(:detail_mode, nil)
     |> assign(:detail_record, nil)
     |> assign(:form, to_form(Settings.change_master_problem_template(%MasterProblemTemplate{})))
  ...
  defp index_path(%{table_query_string: ""}), do: ~p"/setting/patient/problem_templates"

  defp index_path(%{table_query_string: qs}),
    do: "/setting/patient/problem_templates?" <> qs

  defp load_template_detail(id) do
    Settings.get_master_problem_template!(id)
  ...
  def handle_event("open_detail", %{"id" => id}, socket) do
    ...
    m = load_template_detail(id)
    cs = Settings.change_master_problem_template(m)

    {:noreply,
     socket
     |> assign(:detail_record, m)
  ...
  def handle_event("validate", %{"master_problem_template" => params}, socket) do
    changeset =
      %MasterProblemTemplate{}
      |> Settings.change_master_problem_template(params)
      |> Map.put(:action, :validate)

    {:noreply, assign(socket, :form, to_form(changeset))}
  end

  def handle_event("create", %{"master_problem_template" => params}, socket) do
    case Settings.create_master_problem_template(params, socket.assigns.current_user.id) do
      {:ok, _} ->
  ...
  def handle_event("validate_detail", %{"master_problem_template" => params}, socket) do
    if socket.assigns.detail_mode != :edit do
      {:noreply, socket}
    else
      m = socket.assigns.detail_record
  ...
  def handle_event("update_detail", %{"master_problem_template" => params}, socket) do
    if socket.assigns.detail_mode != :edit do
      {:noreply, socket}
    else
      m = socket.assigns.detail_record

      case Settings.update_master_problem_template(m, params, socket.assigns.current_user.id) do
        {:ok, _} ->
          to = index_path(socket.assigns)
  ...
  def handle_event("archive", %{"id" => id}, socket) do
    id = String.to_integer(id)
    m = Settings.get_master_problem_template!(id)

    case Settings.archive_master_problem_template(m, socket.assigns.current_user.id) do
      {:ok, _} ->
        to = index_path(socket.assigns)
        ...
        socket =
          if match?(%MasterProblemTemplate{}, socket.assigns.detail_record) &&
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, "Could not archive this template.")}
    end
  end
  ...
  def handle_event("restore", %{"id" => id}, socket) do
    ...
    case Settings.restore_master_problem_template(m, socket.assigns.current_user.id) do
      {:ok, _} ->
        to = index_path(socket.assigns)
        {:noreply, socket |> put_flash(:info, "Template restored.") |> push_patch(to: to)}

      {:error, _} ->
        {:noreply, put_flash(socket, :error, "Could not restore this template.")}
    end
  end
  ...
  def render(assigns) do
    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <.header>
        Master problem templates
        <:subtitle>
          Manage reusable templates (archived items are hidden from patient pickers)
        </:subtitle>
        <:actions>
      ...
      <.drawer id={@new_drawer_id} on_cancel={hide_drawer(@new_drawer_id)}>
        <div class="space-y-4">
          ...
          <p id={"#{@new_drawer_id}-description"} class="text-sm text-gray-600">
            Add a name and description. You can archive it later if it is no longer needed.
          </p>
          <.simple_form
            for={@form}
            class="space-y-4"
