defmodule SnippetSaverWeb.SettingLive.SpeciesLive do
  use SnippetSaverWeb, :live_view

  use LiveTable.ExportHelpers, schema: SnippetSaver.Settings.Species
  ...
  use LiveTable.TableComponent,
    table_options:
      LiveTable.TableConfig.get_table_options(
        SnippetSaverWeb.SettingLive.SpeciesTable.table_options()
      )
  ...
  alias SnippetSaverWeb.SettingLive.{CatalogTableSupport, SpeciesTable}

  def fields, do: SpeciesTable.fields()
  def filters, do: SpeciesTable.filters()

  def list_resources(fields, options, schema \\ Species) do
    CatalogTableSupport.list_resources(fields, options, schema, SpeciesTable.table_options())
  end

  def stream_resources(fields, options, schema \\ Species) do
    CatalogTableSupport.stream_resources(fields, options, schema, SpeciesTable.table_options())
  end

  def get_merged_table_options do
    CatalogTableSupport.get_merged_table_options(SpeciesTable.table_options())
  end

  @new_drawer_id "new-species-drawer"
  @view_drawer_id "view-species-drawer"
  ...
  def mount(_params, _session, socket) do
    {:ok,
     ...
     |> assign(:form, to_form(Settings.change_species(%Species{})))
  ...
  defp index_path(%{table_query_string: ""}), do: ~p"/setting/patient/species"

  defp index_path(%{table_query_string: qs}),
    do: "/setting/patient/species?" <> qs

  defp load_species_detail(id) do
    Settings.get_species!(id) |> Repo.preload([:inserted_by, :updated_by])
  end
  ...
  def handle_event("open_detail", %{"id" => id}, socket) do
    ...
    sp = load_species_detail(id)
    cs = Settings.change_species(sp)

    {:noreply,
     socket
     |> assign(:detail_record, sp)
  ...
  def handle_event("validate", %{"species" => params}, socket) do
    changeset =
      %Species{}
      |> Settings.change_species(params)
      |> Map.put(:action, :validate)

    {:noreply, assign(socket, :form, to_form(changeset))}
  end

  def handle_event("create", %{"species" => params}, socket) do
    case Settings.create_species(params, socket.assigns.current_user.id) do
      {:ok, _} ->
  ...
  def handle_event("validate_detail", %{"species" => params}, socket) do
    if socket.assigns.detail_mode != :edit do
      {:noreply, socket}
    else
      sp = socket.assigns.detail_record
  ...
  def handle_event("update_detail", %{"species" => params}, socket) do
    if socket.assigns.detail_mode != :edit do
      {:noreply, socket}
    else
      sp = socket.assigns.detail_record

      case Settings.update_species(sp, params, socket.assigns.current_user.id) do
        {:ok, _} ->
          to = index_path(socket.assigns)
  ...
  def handle_event("archive", %{"id" => id}, socket) do
    id = String.to_integer(id)
    sp = Settings.get_species!(id)

    case Settings.archive_species(sp, socket.assigns.current_user.id) do
      {:ok, _} ->
        to = index_path(socket.assigns)

        socket =
          socket
          |> put_flash(:info, "Species archived.")

        socket =
          if match?(%Species{}, socket.assigns.detail_record) &&
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, "Could not archive this species.")}
    end
  end
  ...
  def handle_event("restore", %{"id" => id}, socket) do
    ...
    case Settings.restore_species(sp, socket.assigns.current_user.id) do
      {:ok, _} ->
        to = index_path(socket.assigns)
        {:noreply, socket |> put_flash(:info, "Species restored.") |> push_patch(to: to)}

      {:error, _} ->
        {:noreply, put_flash(socket, :error, "Could not restore this species.")}
    end
  end
  ...
  def render(assigns) do
    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <.header>
        Species
        <:subtitle>Manage species (archived items are hidden from patient pickers)</:subtitle>
        <:actions>
      ...
      <.drawer id={@new_drawer_id} on_cancel={hide_drawer(@new_drawer_id)}>
        <div class="space-y-4">
          ...
          <p id={"#{@new_drawer_id}-description"} class="text-sm text-gray-600">
            Add a name for this species. You can archive it later if it is no longer needed.
          </p>
