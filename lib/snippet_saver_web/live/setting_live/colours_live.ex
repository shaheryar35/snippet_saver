defmodule SnippetSaverWeb.SettingLive.ColoursLive do
  use SnippetSaverWeb, :live_view

  use LiveTable.ExportHelpers, schema: SnippetSaver.Settings.Colour
  ...
  use LiveTable.TableComponent,
    table_options:
      LiveTable.TableConfig.get_table_options(
        SnippetSaverWeb.SettingLive.ColourTable.table_options()
      )
  ...
  alias SnippetSaverWeb.SettingLive.{CatalogTableSupport, ColourTable}

  def fields, do: ColourTable.fields()
  def filters, do: ColourTable.filters()

  def list_resources(fields, options, schema \\ Colour) do
    CatalogTableSupport.list_resources(fields, options, schema, ColourTable.table_options())
  end

  def stream_resources(fields, options, schema \\ Colour) do
    CatalogTableSupport.stream_resources(fields, options, schema, ColourTable.table_options())
  end

  def get_merged_table_options do
    CatalogTableSupport.get_merged_table_options(ColourTable.table_options())
  end

  @new_drawer_id "new-colour-drawer"
  @view_drawer_id "view-colour-drawer"

  @impl true
  def mount(_params, _session, socket) do
    {:ok,
     socket
     |> assign(:page_title, "Colours")
     |> assign(:active_page, "settings_patient")
     |> assign(:detail_form, nil)
     |> assign(:detail_mode, nil)
     |> assign(:detail_record, nil)
     |> assign(:form, to_form(Settings.change_colour(%Colour{})))
  ...
  defp index_path(%{table_query_string: ""}), do: ~p"/setting/patient/colours"
  defp index_path(%{table_query_string: qs}), do: "/setting/patient/colours?" <> qs

  defp load_colour_detail(id) do
    Settings.get_colour!(id) |> Repo.preload([:inserted_by, :updated_by])
  end
  ...
  def handle_event("open_detail", %{"id" => id}, socket) do
    ...
    c = load_colour_detail(id)
    cs = Settings.change_colour(c)

    {:noreply,
     socket
     |> assign(:detail_record, c)
  ...
  def handle_event("validate", %{"colour" => params}, socket) do
    changeset =
      %Colour{}
      |> Settings.change_colour(params)
      |> Map.put(:action, :validate)

    {:noreply, assign(socket, :form, to_form(changeset))}
  end

  def handle_event("create", %{"colour" => params}, socket) do
    case Settings.create_colour(params, socket.assigns.current_user.id) do
      {:ok, _} ->
        to = index_path(socket.assigns)

        {:noreply,
         socket
         |> put_flash(:info, "Colour created.")
         |> assign(:form, to_form(Settings.change_colour(%Colour{})))
         |> push_patch(to: to)}
  ...
  def handle_event("validate_detail", %{"colour" => params}, socket) do
    if socket.assigns.detail_mode != :edit do
      {:noreply, socket}
    else
      c = socket.assigns.detail_record
  ...
  def handle_event("update_detail", %{"colour" => params}, socket) do
    if socket.assigns.detail_mode != :edit do
      {:noreply, socket}
    else
      c = socket.assigns.detail_record

      case Settings.update_colour(c, params, socket.assigns.current_user.id) do
        {:ok, _} ->
          to = index_path(socket.assigns)

          {:noreply,
           socket
           |> put_flash(:info, "Colour updated.")
           |> assign(:detail_form, nil)
           |> assign(:detail_mode, nil)
           |> assign(:detail_record, nil)
           |> push_patch(to: to)}
  ...
  def handle_event("archive", %{"id" => id}, socket) do
    id = String.to_integer(id)
    c = Settings.get_colour!(id)

    case Settings.archive_colour(c, socket.assigns.current_user.id) do
      {:ok, _} ->
        to = index_path(socket.assigns)

        socket =
          socket
          |> put_flash(:info, "Colour archived.")

        socket =
          if match?(%Colour{}, socket.assigns.detail_record) &&
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, "Could not archive this colour.")}
    end
  end
  ...
  def handle_event("restore", %{"id" => id}, socket) do
    ...
    case Settings.restore_colour(c, socket.assigns.current_user.id) do
      {:ok, _} ->
        to = index_path(socket.assigns)
        {:noreply, socket |> put_flash(:info, "Colour restored.") |> push_patch(to: to)}

      {:error, _} ->
        {:noreply, put_flash(socket, :error, "Could not restore this colour.")}
    end
  end
  ...
  def render(assigns) do
    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <.header>
        Colours
        <:subtitle>Manage colours (archived items are hidden from patient pickers)</:subtitle>
        <:actions>
      ...
      <.drawer id={@new_drawer_id} on_cancel={hide_drawer(@new_drawer_id)}>
        <div class="space-y-4">
          ...
          <p id={"#{@new_drawer_id}-description"} class="text-sm text-gray-600">
            Add a name for this colour. You can archive it later if it is no longer needed.
          </p>
