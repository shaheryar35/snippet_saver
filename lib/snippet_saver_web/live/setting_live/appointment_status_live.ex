defmodule SnippetSaverWeb.SettingLive.AppointmentStatusLive do
  use SnippetSaverWeb, :live_view

  use LiveTable.ExportHelpers, schema: SnippetSaver.Settings.AppointmentStatus
  ...
  use LiveTable.TableComponent,
    table_options:
      LiveTable.TableConfig.get_table_options(
        SnippetSaverWeb.SettingLive.AppointmentStatusTable.table_options()
      )
  ...
  alias SnippetSaverWeb.SettingLive.{AppointmentStatusTable, CatalogTableSupport}

  def fields, do: AppointmentStatusTable.fields()
  def filters, do: AppointmentStatusTable.filters()

  def list_resources(fields, options, schema \\ AppointmentStatus) do
    CatalogTableSupport.list_resources(fields, options, schema, AppointmentStatusTable.table_options())
  end

  def stream_resources(fields, options, schema \\ AppointmentStatus) do
    CatalogTableSupport.stream_resources(fields, options, schema, AppointmentStatusTable.table_options())
  end

  def get_merged_table_options do
    CatalogTableSupport.get_merged_table_options(AppointmentStatusTable.table_options())
  end

  @new_drawer_id "new-appointment-status-drawer"
  @view_drawer_id "view-appointment-status-drawer"

  @impl true
  def mount(_params, _session, socket) do
    {:ok,
     socket
     |> assign(:page_title, "Appointment status")
     |> assign(:active_page, "clinic")
     |> assign(:detail_form, nil)
     |> assign(:detail_mode, nil)
     |> assign(:detail_record, nil)
     |> assign(:form, to_form(Settings.change_appointment_status(%AppointmentStatus{})))
  ...
  defp index_path(%{table_query_string: ""}), do: ~p"/setting/clinic/appointment_status"

  defp index_path(%{table_query_string: qs}), do: "/setting/clinic/appointment_status?" <> qs

  defp load_appointment_status_detail(id) do
    Settings.get_appointment_status!(id) |> Repo.preload([:inserted_by, :updated_by])
  end
  ...
  def handle_event("open_detail", %{"id" => id}, socket) do
    ...
    s = load_appointment_status_detail(id)
    cs = Settings.change_appointment_status(s)

    {:noreply,
     socket
     |> assign(:detail_record, s)
  ...
  def handle_event("validate", %{"appointment_status" => params}, socket) do
    changeset =
      %AppointmentStatus{}
      |> Settings.change_appointment_status(params)
      |> Map.put(:action, :validate)

    {:noreply, assign(socket, :form, to_form(changeset))}
  end

  def handle_event("create", %{"appointment_status" => params}, socket) do
    case Settings.create_appointment_status(params, socket.assigns.current_user.id) do
      {:ok, _} ->
        to = index_path(socket.assigns)

        {:noreply,
         socket
         |> put_flash(:info, "Appointment status created.")
         |> assign(:form, to_form(Settings.change_appointment_status(%AppointmentStatus{})))
         |> push_patch(to: to)}
  ...
  def handle_event("validate_detail", %{"appointment_status" => params}, socket) do
    if socket.assigns.detail_mode != :edit do
      {:noreply, socket}
    else
      s = socket.assigns.detail_record
  ...
  def handle_event("update_detail", %{"appointment_status" => params}, socket) do
    if socket.assigns.detail_mode != :edit do
      {:noreply, socket}
    else
      s = socket.assigns.detail_record

      case Settings.update_appointment_status(s, params, socket.assigns.current_user.id) do
        {:ok, _} ->
          to = index_path(socket.assigns)

          {:noreply,
           socket
           |> put_flash(:info, "Appointment status updated.")
           |> assign(:detail_form, nil)
           |> assign(:detail_mode, nil)
           |> assign(:detail_record, nil)
           |> push_patch(to: to)}
  ...
  def handle_event("archive", %{"id" => id}, socket) do
    id = String.to_integer(id)
    s = Settings.get_appointment_status!(id)

    case Settings.archive_appointment_status(s, socket.assigns.current_user.id) do
      {:ok, _} ->
        to = index_path(socket.assigns)

        socket =
          socket
          |> put_flash(:info, "Appointment status archived.")

        socket =
          if match?(%AppointmentStatus{}, socket.assigns.detail_record) &&
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, "Could not archive this appointment status.")}
    end
  end
  ...
  def handle_event("restore", %{"id" => id}, socket) do
    ...
    case Settings.restore_appointment_status(s, socket.assigns.current_user.id) do
      {:ok, _} ->
        to = index_path(socket.assigns)
        {:noreply, socket |> put_flash(:info, "Appointment status restored.") |> push_patch(to: to)}

      {:error, _} ->
        {:noreply, put_flash(socket, :error, "Could not restore this appointment status.")}
    end
  end
  ...
  def render(assigns) do
    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <.header>
        Appointment status
        <:subtitle>
          Manage status labels and colors for appointments. Archived items are hidden from pickers.
        </:subtitle>
        <:actions>
          ...
          >
            <div class="grid grid-cols-1 gap-4">
              ...
              <.input
                ...
                type="text"
                label="Color (CSS value, e.g. #22c55e)"
                required={@detail_mode == :edit}
                readonly={@detail_mode == :view}
              />
            </div>
      ...
      <.drawer id={@new_drawer_id} on_cancel={hide_drawer(@new_drawer_id)}>
        <div class="space-y-4">
          ...
          <p id={"#{@new_drawer_id}-description"} class="text-sm text-gray-600">
            Add a name and display color. You can archive it later if it is no longer needed.
          </p>
          <.simple_form for={@form} class="space-y-4" phx-change="validate" phx-submit="create">
            <div class="grid grid-cols-1 gap-4">
              <.input field={@form[:name]} type="text" label="Name" required />
              <.input field={@form[:color]} type="text" label="Color (CSS value, e.g. #22c55e)" required />
            </div>
            <:actions>
