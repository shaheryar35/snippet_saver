defmodule SnippetSaverWeb.SettingLive.ContactRoleTypeTable do
  @moduledoc """
  LiveTable configuration for contact role types (settings): fields, filters, options.
  """
    ...
    [
      ...
      name: %{
        ...
        renderer: fn name, row ->
          rt = Map.get(row, :actions)
          archived? = rt && rt.archived == true
      ...
      actions: %{
        ...
        renderer: fn rt ->
          id = if is_struct(rt), do: rt.id, else: rt[:id] || get_in(rt, [:actions, :id])
          archived? = if is_struct(rt), do: rt.archived, else: Map.get(rt, :archived, false)
          ...
          <div class="flex flex-wrap items-center justify-end gap-2">
            ...
            <.button
              ...
              phx-click="archive"
              phx-value-id={@id}
              data-confirm="Archive this role type? It will be hidden from pickers."
            >
